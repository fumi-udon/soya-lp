<div id="checkout-modal" class="fixed inset-0 bg-[#2c3034]/70 backdrop-blur-[4px] z-[2000] hidden flex-col justify-end">
    <div id="checkout-sheet"
        ...
        <div
            ...
            {{-- Submit to API; WhatsApp confirmation follows on the next step --}}
            <button id="submit-order-btn" type="button" onclick="App.submitOrder()"
                class="group w-full bg-[var(--theme-primary)] text-white py-4 px-4 rounded-2xl font-bold text-[13px] tracking-wide transition-all shadow-lg shadow-[var(--theme-primary)]/30 hover:brightness-95 active:scale-[0.99] flex justify-center items-center gap-3">
                <svg class="h-6 w-6 shrink-0 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor"
